...
func ReadHeader(conn net.Conn, protocol *TinReqProtocol) error {
...
func ReadBody(conn net.Conn, protocol *TinReqProtocol) error {
	bodyLength := int(protocol.GetHeader().BodyLength)
	bytes := make([]byte, bodyLength)

	totalRead := 0
	for totalRead < bodyLength {
		n, err := conn.Read(bytes[totalRead:])
		if err != nil {
			return err
		}
		totalRead += n
...
func ReadTail(conn net.Conn, protocol *TinReqProtocol) error {
